...
-- Write SQL query here
-- First, get the city name from the previous clue
WITH target_city AS (
    SELECT c.name
    FROM cities c
    JOIN countries co ON c.countrycode = co.code
...
WHERE co.continent = 'South America'
  AND ci.name LIKE (SELECT prefix FROM prefix) || '%'
  AND ci.name <> (SELECT name FROM target_city);

-- Clue #6: We're close! Our South American agent says she just got a taxi at the airport, and is headed towards
-- the capital! Look up the country's capital, and get there pronto! Send us the name of where you're headed and we'll
-- follow right behind you!
-- Write SQL query here
-- Step 1: Get the country code of the city found in Clue #5
WITH target_city AS (
    SELECT c.name
    FROM cities c
    JOIN countries co ON c.countrycode = co.code
...
found_city AS (
    SELECT ci.*, co.code AS country_code
    FROM cities ci
    JOIN countries co ON ci.countrycode = co.code
    WHERE co.continent = 'South America'
      AND ci.name LIKE (SELECT prefix FROM prefix) || '%'
...
-- Step 2: Get the capital city of the country
SELECT cap.*
FROM countries co
JOIN cities cap ON co.capital = cap.id
WHERE co.code = (SELECT country_code FROM found_city);
